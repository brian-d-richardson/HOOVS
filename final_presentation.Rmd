...
for (fold in 1:k){
  ...
  rf<-randomforest.features("y",fold_train)
  toc()
  
  #Predict using random forest trained on selected features
  ## extract important features
  imp.features<-rf$important.features
  ## train model on selected features
  model<-ordinalForest::ordfor("y", fold_train[,c("y",imp.features)])
  ## save predictions on cross validation test set
  predicted <- predict(model,fold_test[,-1])
    
  #Calculate Kappa
  test_kappa <- cohen.kappa(x = matrix(c(fold_test[,1], predicted[-length(predicted)]),
                                             ncol = 2, 
                                             byrow = F))$weighted.kappa
    
  
  
  #Record Kappa for the fold
  best_kappa[fold] <- test_kappa
  #Record list of important variables for the fold
  important_vars<- append(important_vars, list(imp.features))
}
