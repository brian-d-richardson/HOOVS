...
plt_important_cat_features<-function(df,feature,file_location){
  ...
  # save plot as png
  ggsave(paste0(file_location,sub(":.*", "",feature),"_plot.png"),plt,width=10,height=6)
}


for(f in imp.features){
  # check if variable is categorical
  if(nrow(unique(data[,..f]))>10){
    plt_important_cat_features(data,f,"rf plots/")}
  
}

plt_important_cat_features(data,"B7:SATISFACTION W/HOME","rf plots/")
for(f in imp.features){
  if(nrow(unique(data[,..f]))>10){
    print(f)}
}
