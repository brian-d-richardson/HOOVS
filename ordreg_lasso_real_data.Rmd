...
p1 <- ggplot(lambda_info_df, aes(x = n.nonzero, y = testing_kappa, group = fold)) +
  ...
  labs(title = "No. of Nonzero Betas vs. Testing Kappa by Fold",
       x = "No. of Nonzero Betas",
       y = "Testing Kappa",
       color = "Fold")
...
p2 <- ggplot(data=lambda_avg, aes(x = avg_n.nonzero, y = avg_testing_kappa, label = lambda)) +
  ...
  labs(title = "Average No. of Nonzero Betas vs. Average Testing Kappa across Fold",
       x = "Average No. of Nonzero Betas",
...
print(paste0("Kappa of the model in held out data is ", round(ordreg.final$kappa, digits = 3)))
...
signif_tbl <- data.frame(Question = colnames(data_heldout_filter[-1]),
                         ...
                         SE = unname(diag(ordreg.final$cov[5:43,5:43])))
