---
title: "Ordinal Regression Lasso"
...
# Load all packages and set up

If the packages is missing, the following code will install and load all the packages.
...
We begin with a theoretical introduction of the ordinal regression model and how our package calculates the estimates and updates at each step. 
...
# Implementation with our Package

## Data Generation

Our package allows for the user to simulate their own data with the `simulate.data()` function. For $n$ subjects, we generate $p$ covariates from independent standard normal distributions^[Our current version can only handle continuous covariates, and categorical varaibles must be created into dummy variables by hand.]. Given true parameters $\pmb{\alpha}_0$ and $\pmb{\beta}_0$, we compute the multinomial probabilities for the outcome for each individual and simulate $y_i$ accordingly.
