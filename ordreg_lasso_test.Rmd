...
The log-likelihood can be maximized numerically using the BFGS method in `optim`. To ensure that the MLEs for $\pmb{\alpha}$ satisfy the constraint $\alpha_1 \leq \dots \leq \alpha_{J-1}$, we can reparametrize the model with $\pmb{\zeta} = (\zeta_1, \dots, \zeta_{J-1})^T$, where $\zeta_1 = \alpha_1$ and $\zeta_j = \log(\alpha_j - \alpha_{j-1})$ for $j = 2, \dots, J-1$.
